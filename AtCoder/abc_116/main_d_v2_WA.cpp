...
//    cin.tie(NULL);
    ...
    uint i=0;
    for(; vTD_rm.size()<K; ++i){
        if( htUN[ vTD[i].t ]==0 ){ // <- Candidate of Bug: more than 2 val needs to be used when the K is enought big.
...
//        if( htUN[ vTD[i].t ] != 0 ){ continue; }
    ...
    int64 numT = K; // num of type
    int64 point = sum + numT * numT;
    uint ri=0, ai=0;
    while( ri<vTD_rm.size() && ai<vTD_add.size() ){
//        if( vTD_rm[ri].t == vTD_add[ai].t ){ ++ai; continue; }
//        if( htUN[ vTD_add[i].t ]!=0 ){ ++ai; continue; }
